...
const getPricesFromDb = function (pet, db) {
  ...
  if (pet.skin) {
    lvl1 =
      db[`lvl_1_${pet.tier}_${pet.type}_skinned_${pet.skin}`.toLowerCase()]
        ?.price || lvl1;
    lvl100 =
      db[`lvl_100_${pet.tier}_${pet.type}_skinned_${pet.skin}`.toLowerCase()]
        ?.price || lvl100;
    lvl200 =
      db[`lvl_200_${pet.tier}_${pet.type}_skinned_${pet.skin}`.toLowerCase()]
        ?.price || lvl200;
  }
...
const getPetPrice = function (pet, db) {
  ...
  pet.price = price;
  pet.modified = {
    name: `[Lvl ${data.level}] ${helper.capitalize(`${pet.tier} ${pet.type}`)}${
      pet.skin ? " ✦" : ""
    }`,
    isPet: true,
  };
