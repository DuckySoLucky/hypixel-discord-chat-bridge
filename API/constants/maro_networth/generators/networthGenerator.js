...
const getNetworth = async function (data, profile, bank) {
  ...
  for (const key of Object.keys(data)) {
    ...
    for (const item of data[key].filter((i) => i.price)) {
      const isSoulbound =
        item.tag?.ExtraAttributes?.donated_museum ||
    ...
    if (category.unsoulbound_items.length > 0) {
      ...
      output.categories[key].unsoulbound_total = parseInt(
        category.unsoulbound_total
      );
  ...
  output.bank = bank || 0;
  output.personal_bank =
    (profile.bank_account || 0) !== (bank || 0) ? profile.bank_account || 0 : 0;
  ...
  output.networth =
    ...
    output.personal_bank +
    output.purse;
  output.unsoulbound_networth =
    Object.values(output.categories).reduce(
      (a, b) => a + (b.unsoulbound_total || 0),
      0
    ) +
    output.bank +
    output.personal_bank +
