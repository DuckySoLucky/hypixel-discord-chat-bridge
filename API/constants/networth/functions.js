...
const format_item_name = (
  name,
  item,
  { pet = false, tier = "common", level = null, tierboosted = false } = {}
) => {
  ...
  if (pet) {
    if (!level) {
      ...
      name = name
        .replace(/\[lvl ?([0-9]*)] ?/gi, `${level}:`)
        .replace(/_/g, " ");
      name = `${tier}:${name}`;
    } else
      name = `${tier}:${level}:${name}${tierboosted ? ":t" : ""}`.replace(
        /_/g,
        " "
      );
  } else {
  ...
  getPrice: function getPrice(item, prices, pet = false) {
    try {
      ...
      if (pet) {
        ...
        if (tierboosted) {
          item.tier =
            pet_raritys[pet_raritys.indexOf(item.tier.toLowerCase()) + 1];
        }
        name = format_item_name(item.type.toLowerCase(), item, {
          pet: true,
      ...
      let key = Object.keys(prices).includes(name)
        ? name
        : Object.keys(prices).includes(item.tag?.ExtraAttributes.id)
        ? item.tag?.ExtraAttributes.id
        : null;
      let val = 0;
      if (key == null) {
        if (
          item.tag.display.Lore[item.tag.display.Lore.length - 1].includes(
            "HOE"
          ) &&
          item.tag.ExtraAttributes.id.startsWith("THEORETICAL")
        ) {
          const hoe = getHoe(item.tag.ExtraAttributes.id);
          const tickets =
            hoe.level == 2
              ? prices["JACOBS_TICKET"] * 64
              : prices["JACOBS_TICKET"] * 256;
          val =
            1000000 +
            256 * (prices[hoe.material] * 144 ** (hoe.level - 1)) +
            tickets;
        } else {
          return {
      ...
      if (!pet) {
        ...
        if (name.includes("midas staff")) {
          ...
          extra_information.push({
            type: "Winning Price",
            value: item.tag.ExtraAttributes?.winning_bid || 0,
            amount: 1,
          });
        ...
        if (item.tag.ExtraAttributes.hot_potato_count) {
          if (item.tag.ExtraAttributes.hot_potato_count > 10) {
            ...
            val +=
              prices["FUMING_POTATO_BOOK"] ||
              0 * item.tag.ExtraAttributes.hot_potato_count - 10;
            extra_information.push({
              type: "Hot Potato Book",
              value: (prices["HOT_POTATO_BOOK"] || 0) * 10,
              amount: 10,
            });
            extra_information.push({
              type: "Fuming Potato Book",
              value:
                (prices["FUMING_POTATO_BOOK"] || 0) *
                  item.tag.ExtraAttributes.hot_potato_count -
                10,
              amount: item.tag.ExtraAttributes.hot_potato_count - 10,
            });
          } else {
            val +=
              prices["HOT_POTATO_BOOK"] ||
              0 * item.tag.ExtraAttributes.hot_potato_count;
            extra_information.push({
              type: "Hot Potato Book",
              value:
                (prices["HOT_POTATO_BOOK"] || 0) *
                item.tag.ExtraAttributes.hot_potato_count,
              amount: item.tag.ExtraAttributes.hot_potato_count,
            });
          }
        }
        ...
        if (item.tag.ExtraAttributes.art_of_war_count) {
          val +=
            prices["the art of war"] ||
            0 * item.tag.ExtraAttributes.art_of_war_count;
          extra_information.push({
            type: "Art of War",
            value:
              (prices["the art of war"] || 0) *
              item.tag.ExtraAttributes.art_of_war_count,
            amount: item.tag.ExtraAttributes.art_of_war_count,
          });
        }
        ...
        if (item.tag.ExtraAttributes.farming_for_dummies_count) {
          val +=
            prices["farming for dummies"] ||
            0 ||
            0 * item.tag.ExtraAttributes.farming_for_dummies_count;
          extra_information.push({
            type: "Farming for Dummies",
            value:
              (prices["farming for dummies"] || 0) *
              item.tag.ExtraAttributes.farming_for_dummies_count,
            amount: item.tag.ExtraAttributes.farming_for_dummies_count,
          });
        }

        //REFORGE
        let reforge = item.tag.display.Name.substring(
          0,
          item.tag.display.Name.indexOf(" ")
        );
        reforge = reforge
          .replace(/✪/g, "")
          .replace(/\[lvl ?[0-9]*]/gi, "")
        ...
        if (reforge_stones[reforge]) {
          ...
          extra_information.push({
            type: `${reforge} Reforge`,
            value: prices[reforge_stones[reforge].item_name.toLowerCase()] || 0,
            amount: 1,
          });
        }
        ...
        if (item.tag.ExtraAttributes.enchantments) {
          for (const enchantment of Object.keys(
            item.tag.ExtraAttributes.enchantments
          )) {
            let name =
              `${enchantment} ${item.tag.ExtraAttributes.enchantments[enchantment]}`
                .toLowerCase()
                .replace(/_/g, " ");
            if (ignored_enchantments.includes(name)) {
              ...
              if (
                !item.tag.display.Name.toLowerCase().includes(
                  "ice spray wand"
                ) &&
                !name.includes("scavenger 5")
              ) {
                if (
                  !item.tag.display?.Name.toLowerCase().includes("stonk") &&
                  enchantment === "efficiency" &&
                  item.tag.ExtraAttributes?.enchantments[enchantment] > 5
                ) {
        ...
        if (enchantments.length > 0)
          extra_information.push({
            ...
            value: enchantment_value,
            amount: enchantments.length,
          });

        //SCROLLS (Necron's Blade)
        if (
          name.includes("scylla") ||
          name.includes("astraea") ||
          ...
          if (item.tag.ExtraAttributes.ability_scroll) {
            for (let scroll of item.tag.ExtraAttributes.ability_scroll) {
              scroll = scroll
                .replace("WITHER_SHIELD_SCROLL", "wither shield")
                .replace("SHADOW_WARP_SCROLL", "shadow warp")
                .replace("IMPLOSION_SCROLL", "implosion");
              val += prices[scroll] || 0;
              extra_information.push({
                type: titleCase(scroll),
                value: prices[scroll] || 0,
                amount: 1,
              });
            }
          }
        ...
        //DUNGEON STARS
        if (
          item.tag.ExtraAttributes?.dungeon_item_level &&
          item.tag.ExtraAttributes?.id
        ) {
          ...
          if (essence_upgrades[id]) {
            ...
            for (
              let i = 0;
              i <= item.tag.ExtraAttributes.dungeon_item_level && i <= 5;
              i++
            ) {
            ...
            extra_information.push({
              type: `${essence_item.type} Essence`,
              value: essence_amount * essence_prices[essence_item.type],
            ...
            if (item.tag.ExtraAttributes.dungeon_item_level > 5) {
              for (
                let i = 0;
                i < item.tag.ExtraAttributes.dungeon_item_level - 5;
                i++
              ) {
                val += prices[master_stars[i]] || 0;
                extra_information.push({
                  type: titleCase(master_stars[i]),
                  value: prices[master_stars[i]] || 0,
                  amount: 1,
                });
              }
            }
        ...
        if (item.tag.ExtraAttributes.skin) {
          ...
          extra_information.push({
            type: titleCase(item.tag.ExtraAttributes.skin.replace(/_/g, " ")),
            value: prices[item.tag.ExtraAttributes.skin] || 0,
            amount: 1,
          });
        }
        ...
        if (item.tag.ExtraAttributes.talisman_enrichment) {
          val +=
            prices[
              item.tag.ExtraAttributes.talisman_enrichment
                .toLowerCase()
                .replace(/_/g, " ") + " enrichment"
            ] || 0;
          extra_information.push({
            type: titleCase(
              item.tag.ExtraAttributes.talisman_enrichment.replace(/_/g, " ") +
                " Enrichment"
            ),
            value:
              prices[
        ...
        if (item.tag.ExtraAttributes?.gems) {
          for (const gem of Object.keys(item.tag.ExtraAttributes.gems)) {
            val +=
              prices[
                `${item.tag.ExtraAttributes.gems[gem]}_${gem.substring(
                  0,
                  gem.length - 2
                )}_GEM`
              ] || 0;
            if (
              ...
              extra_information.push({
                type: `${titleCase(gem.substring(0, gem.length - 2))} Gemstone`,
                value:
                  prices[
                    `${item.tag.ExtraAttributes.gems[gem]}_${gem.substring(
                      0,
        ...
        if (item.tag.ExtraAttributes.wood_singularity_count) {
          ...
          extra_information.push({
            ...
            value: prices["wood singularity"] || 0,
            amount: 1,
          });
        }
        ...
        if (item.tag.ExtraAttributes.tuned_transmission) {
          val +=
            prices["transmission tuner"] ||
            0 * item.tag.ExtraAttributes.tuned_transmission;
          extra_information.push({
            type: "Wood Singularity",
            value:
              (prices["wood singularity"] || 0) *
              item.tag.ExtraAttributes.tuned_transmission,
            amount: 1,
          });
        }
        ...
        if (item.tag.ExtraAttributes.ethermerge) {
          val +=
            prices["etherwarp conduit"] || 0 + prices["etherwarp merger"] || 0;
          extra_information.push({
            type: "Ether Transmission",
            value:
              prices["etherwarp conduit"] ||
              0 + prices["etherwarp merger"] ||
              0,
            amount: 1,
          });
        }
        ...
        if (item.tag.ExtraAttributes.drill_part_upgrade_module) {
          let price =
            prices[
              forgeItemTimes[
                item.tag.ExtraAttributes.drill_part_upgrade_module.toUpperCase()
              ].name.toLowerCase()
            ] || 0;
          val += price;
          extra_information.push({
        ...
        if (item.tag.ExtraAttributes.drill_part_engine) {
          let price =
            prices[
              forgeItemTimes[
                item.tag.ExtraAttributes.drill_part_engine.toUpperCase()
              ].name.toLowerCase()
            ] || 0;
          val += price;
          extra_information.push({
        ...
        if (item.tag.ExtraAttributes.drill_part_fuel_tank) {
          let price =
            prices[
              forgeItemTimes[
                item.tag.ExtraAttributes.drill_part_fuel_tank.toUpperCase()
              ].name.toLowerCase()
            ] || 0;
          val += price;
          extra_information.push({
        ...
        if (val > 600000 || item.tag.display.Lore.includes("ACCESSORY")) {
          val += item.tag.ExtraAttributes.rarity_upgrades
            ? prices["RECOMBOBULATOR_3000"] || 0
            : 0;
          if (item.tag.ExtraAttributes.rarity_upgrades)
            extra_information.push({
              type: "Recombobulator 3000",
              value: prices["RECOMBOBULATOR_3000"] || 0,
              amount: 1,
            });
        }
      } else {
        ...
        if (item?.skin) {
          ...
          extra_information.push({
            type: titleCase(item.skin + " Skin"),
            value: getBazaarPrice(`PET_SKIN_${item.skin}`, prices),
            amount: 1,
          });
        }
        //GET PET ITEM
        if (item?.heldItem && pet_items[item?.heldItem]) {
          if (pet_items[item?.heldItem].rarity) {
            val += getBazaarPrice(
              `${pet_items[item?.heldItem].rarity}:${pet_items[
                item.heldItem
              ].item_name.toLowerCase()}`,
              prices
            );
            extra_information.push({
              type: titleCase(pet_items[item.heldItem].item_name),
              value: getBazaarPrice(
                `${pet_items[item?.heldItem].rarity}:${pet_items[
                  item.heldItem
                ].item_name.toLowerCase()}`,
                prices
            ...
            val += getBazaarPrice(
              pet_items[item.heldItem].item_name.toLowerCase(),
              prices
            );
            extra_information.push({
              type: titleCase(pet_items[item.heldItem].item_name),
      ...
      return {
        name: (item.tag?.display?.Name || item?.type || "null").replace(
          /§[0-9a-k]/g,
          ""
        ),
        value: 0,
