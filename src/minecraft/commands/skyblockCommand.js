...
class eightballCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    ...
    if(msg[0]){ 
        const profile = await skyHelperAPI.getProfile(msg[0])
        const skyShiiyuProfile = await skyShiiyuAPI.getProfile(msg[0])
        if (profile.isIronman) msg[0] = `♲ ${msg[0]}`
