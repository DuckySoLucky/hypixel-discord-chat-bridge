...
class dailyCommand extends MinecraftCommand {
    constructor(minecraft) {
        ...
        this.name = 'monthly'
        this.aliases = ['']
        this.description = 'Shows ur monthly stats'
    }
        ...
        try {
            ...
            fetch(`https://api.ashcon.app/mojang/v2/user/${player}`).then((res) => {
                res.json().then((uuid) => {
                    ...
                    fetch(`https://api.pixelic.de/v1/player/monthly?uuid=${UUID}`).then((res) => {
                            ...
                            fetch(`https://api.hypixel.net/player?uuid=${UUID}&key=${config.api.hypixelAPIkey}`).then((res) => {
                                res.json().then((player) => {
                                    ...
                                    if (mode == 'solo') {
                                        ...
                                        this.send(`/gc Monthly stats for ${PlayerName}: Wins: ${bedwars_wins} WLR: ${bedwars_wlr} FKDR: ${bedwars_fkdr} BLR: ${bedwars_bblr} Finals: ${bedwars_finalKills} Beds: ${bedwars_bedsBroken}`)
                                    }
