...
class StateHandler extends eventHandler {
  ...
  async onMessage(event) {
    ...
    if (this.isOnlineInvite(message)) {
      const username = message.replace(/\[(.*?)\]/g, '').trim().split(/ +/g)[2]
      return [this.minecraft.broadcastCleanEmbed({ 
    ...
    if (this.isOfflineInvite(message)) {
      const username = message.replace(/\[(.*?)\]/g, '').trim().split(/ +/g)[6].match(/\w+/g)[0]
      return [this.minecraft.broadcastCleanEmbed({ 
    ...
    if (this.isUserMuteMessage(message)) {
      const username = message.replace(/\[(.*?)\]/g, '').trim().split(/ +/g)[3].replace(/[^\w]+/g, '')
    ...
    if (this.isUserUnmuteMessage(message)) {
      const username = message.replace(/\[(.*?)\]/g, '').trim().split(/ +/g)[3]
      return [ this.minecraft.broadcastCleanEmbed({
    ...
    if (this.isNotInGuild(message)) {
      const username = message.replace(/\[(.*?)\]/g, '').trim().split(' ')[0]
      return this.minecraft.broadcastCleanEmbed({ 
