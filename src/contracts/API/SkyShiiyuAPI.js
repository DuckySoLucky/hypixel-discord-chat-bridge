...
async function getProfile(username) {
    return (await axios.get(`${config.api.skyShiiyuAPI}/profile/${username}`)).data
}

async function getTalismans(username) {
    return (await axios.get(`${config.api.skyShiiyuAPI}/talismans/${username}`)).data.profiles[await getLatestProfileKey(username)]
}

async function getSlayer(username) {
    return (await axios.get(`${config.api.skyShiiyuAPI}/slayers/${username}`)).data
}
...
async function getDungeons(username) {
    return (await axios.get(`${config.api.skyShiiyuAPI}/dungeons/${username}`)).data
}
