...
for (const file of scripts) {
  fixedScripts[`docgen:${lowerFirst(file)}`] = `pnpm exec tsx scripts/generate${file}.ts`;
...
for (const file of docsScripts) {
  fixedScripts[`docgen:docs:${lowerFirst(file.replaceAll(".", ":"))}`] = `pnpm exec tsx scripts/docs/${file}.ts`;
}
